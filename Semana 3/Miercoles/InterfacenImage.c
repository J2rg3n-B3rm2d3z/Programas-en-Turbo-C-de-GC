...
char Direccion[MAXCHAR]="C:\\ImageC\\Fondos\\Shelter.bmp";
...
void main(void)
{
	...
	char DireccionUso[MAXCHAR],Imagen[MAXCHAR];
	...
	while(!Salir)
	{
		...
		muestraBMP(0,0,DireccionUso);	/*Cargar una imagen de fondo*/
		...
		{		
			...
			if(boolmostrar==1)
			{
				...
				if(boton(11,getmaxy()-47,129,getmaxy()-24,NULL,"",0))
				{
					...
					if(ObteDireccion(15,getmaxy()-47))
					{
						strcpy(Imagen,Direccion);
						putimage(15-5-2,getmaxy()-47-20-30,Buffer,COPY_PUT);
						free(Buffer);
						
						muestraBMP(10,10,Imagen);
						
						memset(Imagen,'\0',MAXCHAR);
					}
					else
					{
						putimage(15-5-2,getmaxy()-47-20-30,Buffer,COPY_PUT);
